/**
 * Sky Furniture — All 36 Nigerian states + FCT and their Local Government Areas (LGAs).
 * Used for cascading State → LGA selects on checkout.
 */
...
/** Sorted list of all states (keys). */
function getNigeriaStates() {
  return Object.keys(NIGERIA_LOCATIONS).sort((a, b) => a.localeCompare(b));
}

/** LGAs for a given state name (sorted). */
function getLGAsForState(state) {
  const list = NIGERIA_LOCATIONS[state];
  if (!list) return [];
  return [...list].sort((a, b) => a.localeCompare(b));
}

/** Populate a &lt;select&gt; with Nigeria states. */
function populateStateSelect(selectEl, selected = "Lagos") {
  if (!selectEl) return;
...
/** Populate LGA select from state. */
function populateLGASelect(selectEl, state, selected = "") {
  if (!selectEl) return;
  const lgas = getLGAsForState(state);
  if (!state || !lgas.length) {
    selectEl.innerHTML = '<option value="">Select state first</option>';
  ...
  selectEl.innerHTML =
    ...
    lgas
      .map(
        (lga) =>
          `<option value="${lga}"${lga === selected ? " selected" : ""}>${lga}</option>`
      )
      .join("");
}
